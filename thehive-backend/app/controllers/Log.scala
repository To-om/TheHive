...
class LogCtrl @Inject() (
  ...
  @Timed
  def create(taskId: String) = authenticated(Role.read).async(fieldsBodyParser(logModel)) { implicit request ⇒
  ...
  @Timed
  def update(id: String) = authenticated(Role.write).async(fieldsBodyParser(logModel)) { implicit request ⇒

...
class TaskCtrl @Inject() (
  ...
  @Timed
  def create(caseId: String) = authenticated(Role.write).async(fieldsBodyParser(taskModel)) { implicit request ⇒
  ...
  @Timed
  def update(id: String) = authenticated(Role.write).async(fieldsBodyParser(taskModel)) { implicit request ⇒

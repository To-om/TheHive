...
class ArtifactCtrl @Inject() (
  ...
  @Timed
  def create(caseId: String) = authenticated(Role.write).async(fieldsBodyParser(artifactModel)) { implicit request ⇒
  ...
  @Timed
  def update(id: String) = authenticated(Role.write).async(fieldsBodyParser(artifactModel)) { implicit request ⇒
